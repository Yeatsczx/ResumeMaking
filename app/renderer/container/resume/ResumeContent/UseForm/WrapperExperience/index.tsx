...
interface IProps {
  dataList: any[];
  updateDataList: (newDataList: AdapterExperienceType[]) => void;
  children: React.ReactNode;
}

const WrapperExperience: FC<IProps> = ({ children, dataList, updateDataList }) => {
  const [currentIndex, setCurrentIndex] = useState(-1); // 根据索引从数组 dataList 中获取数据
  const [currentItem, setCurrentItem] = useState<AdapterExperienceType>({}); // 表示当前选中的条目
  const [experienceList, setExperienceList] = useState<AdapterExperienceType[]>([]); // 一个数组，包含多个列表
