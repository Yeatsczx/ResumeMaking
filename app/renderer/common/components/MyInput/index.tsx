...
//Omit表示忽略第一个参数中第二个参数表示的属性。
//React.InputHTMLAttributes<HTMLInputElement>可以把标签的原生属性都继承过来
...
export default class MyInput extends React.PureComponent<InputProps, InputState> {
  ...
  renderBefore() {
    ...
    return !!addonBefore && <div className={styles.my_input_center}>{addonBefore}</div>;
  }

  renderAfter() {
    const { addonAfter } = this.props;
    return !!addonAfter && <div className={styles.my_input_center}>{addonAfter}</div>;
  }

  renderClear() {
    const { allowClear } = this.props;
    return !!allowClear && this.state.text && <i className={styles.my_input_clear} onClick={this.onClear} />;
  }
  ...
  renderTextarea() {
    ...
    return (
      <div className={styles.my_input_textarea} style={{ height: 24 * _rows }}>
        <textarea
          {...{ placeholder, maxLength, id, disabled, autoFocus }}
          rows={_rows}
          value={text}
        ...
        {this.renderClear()}
        {allowCount && (
          <div className={styles.my_input_textarea_footer}>
            <span
              className={cx({
